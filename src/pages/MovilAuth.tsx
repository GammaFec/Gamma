...
const MovilAuth = () => {
  ...
  const [verificationResult, setVerificationResult] = useState<firebase.auth.ConfirmationResult | null>();
  const ID = "MovilAuth";
  ...
  const openValidation = (formPhoneNumber: string) => {
    if (PhoneValidator(formPhoneNumber) && recaptcha) {
      ...
      sendSMSCode(formPhoneNumber, recaptcha).then((result: firebase.auth.ConfirmationResult) => {
  ...
  return (
    <StyledContainer>
      ...
      <StyledMain>
        {validating ?
          <ValidateNumber phoneNumber={phoneNumber ?? ''} goBack={closeValidation} handleSubmit={validateNumberFirebase} /> :
          <RequestNumber handleSubmit={openValidation} phoneNumber={phoneNumber ?? ''} buttonId={ID} />
        }
      </StyledMain>
    </StyledContainer>
  );
